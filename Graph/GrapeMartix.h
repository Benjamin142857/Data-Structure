...
// 创建图(邻接矩阵表示) TODO 优化：创建与矩阵赋值分离，矩阵值先初始化所有为MAX
...
// 清空矩阵/初始化
void GraphMartix_init(PGraphMartix gp) {
    for(int i=0 ; i<gp->node_num ; i++) {
        for(int j=0 ; j<gp->node_num ; j++) {
...
//  以二维坐标的形式打印矩阵
void GraphMartix_print(PGraphMartix gp) {
    for(int j=0 ; j<gp->node_num ; j++) {
...
// 深度优先周游(递归实现)
void GraphMartix_DFS(PGraphMartix gp) {

}


// 广度优先周游(递归实现)
void GraphMartix_BFS(PGraphMartix gp) {
